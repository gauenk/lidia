...
class BatchedLIDIA(nn.Module):
    ...
    def __init__(self, adapt_cfg, pad_offs, arch_opt, lidia_pad=False,
                 ...
                 bs_alpha=0.25, nn_recording=False,
                 nn_record_first_only = False,
    ...
    def forward(self, noisy, flows=None, srch_img=None,
        ...
        noisy += means # restore
        if rescale:
            deno[...]  = self.imax*(deno  * 0.5 + 0.5) # normalize
            noisy[...] = self.imax*(noisy * 0.5 + 0.5) # restore
        return deno
