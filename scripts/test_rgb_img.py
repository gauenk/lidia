...
def run_exp(cfg):
    ...
    # -- network --
    model = lidia.batched.load_model(cfg.sigma,lidia_pad=True).to(cfg.device)
    model.eval()
    ...
    for sample in loader:
        ...
        if run_internal_adapt:
            model.run_internal_adapt(noisy,cfg.sigma,flows=None,
            ...
            for ws,wsdf in adf.groupby("ws"):
                for wt,wtdf in wsdf.groupby("wt"):
                    ...
                    for sigma,sdf in wtdf.groupby("sigma"):
                        ...
                        psnr = sdf['psnr']
                        time = sdf['timer_deno']
                        fields = (sigma,psnr,time)
                        print("[%d]: %2.3f for %2.2f" % fields)
